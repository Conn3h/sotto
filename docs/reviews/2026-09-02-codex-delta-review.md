...
- [P1] Register clipboard restoration before the first suspension — Sources/Sotto/Core/TextInjector.swift:184-184
...
- [P2] Allow representable correction outputs — Sources/SottoDictionary/DictionaryRepresentability.swift:37-40
  For corrections, `write` appears after the first structural arrow, so values beginning with `#` or containing another `->` round-trip correctly; the current unconditional checks reject valid entries such as `x -> y -> z` or `tag -> #swift`, and even prevent editing such entries loaded from disk. [`CLAUDE.md:3-4`](CLAUDE.md#L3-L4) makes the specification contractual, and [`docs/SPEC.md:679-683`](docs/SPEC.md#L679-L683) explicitly defines this first-arrow behavior.

- [P2] Preserve drafts when dictionary mutations are refused — Sources/Sotto/UI/DictionaryPanel.swift:130-130
...
- [P3] Keep canceled timers from clearing replacement handles — Sources/Sotto/UI/HistoryPanel.swift:184-186
